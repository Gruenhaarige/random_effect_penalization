...
# require(MASS)
# generateData(beta = c(1,2,3,4,5), n = 100, covX = diag(4), n_groups = 10, 
#              grV = 1, slope = 0, covU = diag(1), sigma_eps = 1)

# generate_mvrnorm_params <- function(p) {
#   mu <- sample(c(-5:-1, 1:5), size = p, replace = TRUE)
#   A <- matrix(runif(p*p, min = -1, max = 1), nrow = p, ncol = p)
#   B <- (A + t(A)) / 2
#   diagonal_padding <- diag(p) * (p * 2 + 1)
#   sigma <- B + diagonal_padding
#   mu <- round(mu)
#   sigma <- round(sigma, 1)
#   return(list(mu = mu, sigma = sigma))
# }
# 
# mvrnorm_3 <- generate_mvrnorm_params(3)
# mvrnorm(n = 10, mvrnorm_3$mu, mvrnorm_3$sigma)
