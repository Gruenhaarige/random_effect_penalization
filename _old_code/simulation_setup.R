...
generateData <- function(beta, n, covX, n_groups, grV, slope, covU, sigma_eps){
  ...
  X <- MASS::mvrnorm(n, numeric(p), covX)
  X1 <- cbind(rep(1, n), X)
  ...
  if(slope > 0){
    ## deterministic choice of slope variables: x1, x2, ...
    slope_var_names <- paste0("x", seq_len(slope))
  ...
  for(gvar in seq_len(grV)){
    ...
    up <- MASS::mvrnorm(n_groups, numeric(slope + 1), covU)
  ...
  data <- cbind.data.frame(y, X, groups)

  id_fake <- sample(data$G1)
  data$id_fake <- id_fake
